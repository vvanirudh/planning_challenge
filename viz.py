"""
viz -- plotting + build/run helpers for the C++ Rover challenge (Colab).

GENERATED by build_cpp_notebooks.py -- do not hand-edit. show_cpp renders a viz
dump written by the C++ show(); build_and_run compiles a driver, runs it, and
...
def show_cpp(bin_path, title=''):
    ...
    # Scale the colormap to the non-rock terrain range; render rocks via 'over'.
    terrain = grid[~rock]
...
def build_and_run(main_cpp, views, std='c++17', opt='-O2'):
    """Compile `main_cpp` with g++, run ./app, then show_cpp each (bin, title).

    `views` is a list of (bin_file, plot_title). Compiler diagnostics are
    printed verbatim so g++ errors on your planner.hpp/learn.hpp stay visible.
    """
    r = subprocess.run(['g++', opt, '-std=' + std, main_cpp, '-o', 'app'],
