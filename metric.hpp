...
// Resample `path` to ~uniform STEP spacing; fill rounded (rx,ry) and in_bounds.
// Returns false on a degenerate (zero-length) path.
inline bool resample_steps(const World& w, const Path& path,
    ...
    for (int j = 0; j < n; ++j) {
        ...
        // linear interp of x,y at arclength u
...
// Integrate cost_map along path; lower is better, empty path -> +inf.
// Resamples to ~uniform spacing first, so the result is independent of waypoint
// count. Mark "do not enter" cells with ROCK_PENALTY (not inf). Out-of-bounds
// points are charged ROCK_PENALTY too.
...
// cost <= expert bar. Prints a one-line summary; returns the success bool.
// The bar (expert_cost * tol) has comfortable tolerance: an expert-like path
// that avoids the costly terrain clears it -- you don't need an exact match.
inline bool evaluate(const World& w, const Path& path,
    ...
    if (resample_steps(w, path, rx, ry, ib)) {
        ...
        for (size_t j = 0; j < rx.size(); ++j) {
            ...
            double tcv = w.tc(cy,cx);                     // sentinel on rock
            double pen = (tcv >= w.rock_sentinel) ? ROCK_PENALTY : tcv;
            if (pen >= HI) rock_hit = true;
        }
    }
    // honest terrain-only cost: rocks zeroed (we already flagged rock_hit).
    ...
    double bar = w.expert_cost * w.tol;
